...
export default function ProfilePage() {
  ...
  const handleSaveProfile = (data: { fullName: string; email: string; phone: string }) => {
    setUser({
      ...user,
      name: data.fullName,
      email: data.email,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-100 to-pink-50 p-6">
      <div className="max-w-4xl mx-auto">
        ...
        <UserProfileCard user={user} onEdit={() => setEditModalOpen(true)} onDelete={() => setDeleteDialogOpen(true)} />
        <EditProfileModal
          open={editModalOpen}
        ...
        <EditProfileModal open={editModalOpen} onOpenChange={setEditModalOpen} user={{ fullName: user.name, email: user.email, phone: user.phone }} onSave={handleSaveProfile} />
